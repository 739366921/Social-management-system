...
router.post(
  ...
  async (ctx) => {
    ...
    if (ctx.request.body.phone) {
      profilesFields.social.phone = ctx.request.body.phone;
    }
...
router.delete(
  ...
  async (ctx) => {
    ...
    if (profiles[0].experience.length > 0) {
      //找下标元素
      const removeIndex = profiles[0].experience
        .map((item) => item.id)
...
router.delete(
  ...
  async (ctx) => {
    ...
    if (profiles[0].education.length > 0) {
      //找下标元素
      const removeIndex = profiles[0].education
        .map((item) => item.id)
