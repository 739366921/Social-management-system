...
router.post('/',Passport.authenticate('jwt', { session: false }),async ctx =>{
 ...
 if(!isvalid)
 {
    ...
    //存储到数据库里面
    await newPosts.save().then((posts)=>{
        ctx.body=posts
    }).catch(err=>{
        ctx.body=err
    })
...
router.post('/like',Passport.authenticate('jwt', { session: false }),async ctx =>{
        ...
        if(profiles.length>0){
            ...
            const islike=posts.likes.filter(like=>like.user.toString()===ctx.state.user.id).length>0;
...
router.post('/unlike',Passport.authenticate('jwt', { session: false }),async ctx =>{
    ...
    if(profiles.length>0){
        ...
        const unlike=posts.likes.map(item=>item.user.toString()).indexOf(unlike_id)//map()函数，它返回一个新的数组，数组中的元素为原始数组调用函数处理后的值。
...
router.delete('/comments',Passport.authenticate('jwt', { session: false }),async ctx =>{
    ...
    //查找是否已经有评论过
    const posts=await Posts.findById(increase_id)
    const iscomment=posts.comments.filter(comment=>comment._id.toString()===comment_id).length==0
        ...
        const removeIndex=posts.comments.map(item=>item._id.toString()).indexOf(comment_id)
        //删除
